...
public class InputTextMsgDialog extends Dialog {
    ...
    private LiveingActivity mVideoPlayActivity;
    ...
    public InputTextMsgDialog(Context context, int theme, LiveingActivity activity) {
        super(context, theme);
        mContext = context;
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                ...
                switch (keyCode) {
                    case KeyEvent.KEYCODE_ENTER:
                        if (messageTextView.getText().length() > 0) {
                            sendText("" + messageTextView.getText());
                            imm.showSoftInput(rldlgview, InputMethodManager.SHOW_FORCED);
                            imm.hideSoftInputFromWindow(rldlgview.getWindowToken(), 0);
                            dismiss();
                        } else {
                            Toast.makeText(mContext, "输入内容不能为空!", Toast.LENGTH_LONG).show();
                        }
...
//        mVideoPlayActivity.refreshViewAfterDialog();
    ...
    private void sendText(String msg) {
        Log.i("调试发送信息","_____"+"msg:"+msg+"_____");
        Log.i("调试发送信息","_____"+"name:"+LiveMySelfInfo.getInstance().toString()+"_____");
        if (msg.length() == 0)
            return;
        try {
        ...
        Log.i("调试发送信息","_____"+LiveMySelfInfo.getInstance().toString()+"_____");
