...
public class OKhttpHelper {
    ...
    public void getMyRoomId(final Context context) {
        try {
            ...
            myId.put("uid", LiveMySelfInfo.getInstance().getId());
            ...
            if (ret == 0) {
                ...
                SxbLog.d(TAG, LogConstants.ACTION_HOST_CREATE_ROOM + LogConstants.DIV + LiveMySelfInfo.getInstance().getId() + LogConstants.DIV + "request room id"
                        + LogConstants.DIV + LogConstants.STATUS.SUCCEED + LogConstants.DIV + "get room id from local " + LiveMySelfInfo.getInstance().getMyRoomNum());
                LiveMySelfInfo.getInstance().setMyRoomNum(String.valueOf(id));
                //你保存信息
