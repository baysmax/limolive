...
public class PeriscopeLayout extends RelativeLayout {
    ...
    private void init() {
        ...
        Drawable red = getResources().getDrawable(R.drawable.heart0);
        Drawable yellow = getResources().getDrawable(R.drawable.heart1);
        Drawable blue = getResources().getDrawable(R.drawable.heart2);
    ...
    public void addHeart(float x, float y) {
        ...
        Animator set = getAnimator(imageView,x,y);
        set.addListener(new AnimEndListener(imageView));
        set.start();
    }
    
    private Animator getAnimator(View target,float x,float y) {
        AnimatorSet set = getEnterAnimtor(target);

        ValueAnimator bezierValueAnimator = getBezierValueAnimator(target,x,y);
    ...
    private ValueAnimator getBezierValueAnimator(View target,float x,float y) {

        //初始化一个贝塞尔计算器- - 传入
        BezierEvaluator evaluator = new BezierEvaluator(getPointF(2), getPointF(1));

        //这里最好画个图 理解一下 传入了起点 和 终点
        ValueAnimator animator = ValueAnimator.ofObject(evaluator, new PointF(x, y), new PointF(random.nextInt(getWidth()), 0));
   ...
   /* int i=0;
    ...
    public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			addHeart(event.getX(),event.getY());
			return true;
