...
public class LocalImageHelper {
    ...
    public String setCameraImgPath() {
        String foloder= LiveMallApplication.getInstance().getCachePath()
                + "/PostPicture/";
...
//
//            //如果App的权限申请曾经被用户拒绝过，就需要在这里跟用户做出解释 定位
...
//    }
    ...
    public void clear(){
        checkedItems.clear();
        currentSize=(0);
        String foloder= LiveMallApplication.getInstance().getCachePath() + "/PostPicture/";
        File savedir = new File(foloder);
        if (savedir.exists()) {
            deleteFile(savedir);
        }
