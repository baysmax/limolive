package com.example.project.limolive.localalbum.widget;
...
//			view.getParent().bringChildToFront(view);
	...
	public AlbumViewPager(Context context, AttributeSet attrs) {
		...
		//((AppApplication)context.getApplicationContext()).getWindowWidth());
	}
	...
	public class LocalViewPagerAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup viewGroup, int position) {
			...
			ImageLoader.getInstance().displayImage(path.getOriginalUri(), new ImageViewAware(imageView),localOptions, loadingListenerr,new ProcressListener(imageLayout),path.getOrientation());
			return imageLayout;
		}
