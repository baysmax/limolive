...
import static com.example.project.limolive.LiveMallApplication.getApplication;
import static com.example.project.limolive.baidu_location.utils.CommonUtil.dip2px;
import static com.example.project.limolive.baidu_location.utils.CommonUtil.getBitmapSize;
import static com.example.project.limolive.baidu_location.utils.FileUtils.BIG_END;
import static com.example.project.limolive.baidu_location.utils.FileUtils.BIG_SIZE;
...
public class IConJob extends Job {
    ...
    private void downloadIcon(final IconModel iconModel) {
        ...
        if (normalFile.exists() && bigFile.exists()) {
            ...
            getApplication().getEventBus().post(iconEvent);
            //继续看后面还有没有需要下载的
            DownLoadNext();
            return;
        }
        HttpRequest.download(iconModel.getUrl(), saveFile, new FileDownloadCallback() {
            ...
            public void onDone() {
                ...
                iconEvent.seteId(iconModel.getId());
                getApplication().getEventBus().post(iconEvent);
                Log.i("1231额啊大额2312312", "1231额啊大额2312312");
                //继续看后面还有没有需要下载的
                DownLoadNext();
            }
    ...
    private void DownLoadNext() {
        ...
        if (size > (logoUrlList.size() - 1)) {
            ...
            LiveMallApplication.getApplication().getEventBus().post(iconEvent);
        } else {
